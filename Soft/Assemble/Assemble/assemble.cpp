...
public:
	...
	map<word, word> addrMap;	//Êý×éÀïµÄÎ»ÖÃºÍÕæÊµ»ã±àµØÖ·µÄ¶ÔÓ¦±í
	...
	word translate(string reg)	//¼Ä´æÆ÷Ãû, ×ª»»Îª¶þ½øÖÆÊý
	{
		if (reg == "$zero" || reg == "$0")
			return 0;
	...
	bool scan_1()
	{
		...
		while (!fin.eof())
		{
			...
			else if ((c <= 'z' && c >= 'a') || (c <= 'Z' && c >= 'A') || c == '_')
			{
				...
				int i = 0, j = 0, m = 0;	//·Ö¸îÖ¸ÁîÓÃµÄÏÂ±ê

				if (buf[buf.length() - 1] == ':')	//ÊÇ±êºÅ
				{
					...
					realLine--; //È¥³ý¶à¼ÆËãµÄÒ»ÐÐ

					while (1) //½«±êºÅÖ®ºóµ½ÏÂÒ»ÌõÖ¸ÁîÇ°µÄ¿Õ°×ºÍ»Ø³µÆÁ±Î
					{
						c = fin.peek();
						if (c == '\n')
						{
				...
				else
				{
					...
					if (!getline(fin, buf))	//´¦ÀíÕæÖ¸Áî»òÕßÆäËû
					{
						fin.close();
						return 1;
					}
	...
	bool scan_2()
	{
		...
		while (line < codeLen)
		{
			...
			else if (segment == 1)	//data
			{
				...
				{
					for(int i = 3; i < code[line].code.size(); i++)
					{
						code[line].code[2] += code[line].code[i];
					}
			...
			{
				if (code[line].code[0] == "nop") //¸÷ÖÖÎ±Ö¸Áî
				{
					address += CO;
				}
		...
		map<string, word>::iterator it;	//½«ÐéÄâµØÖ·×ª»»ÎªÕæÊµµØÖ·
		...
		while (line < codeLen)
		{
			...
			else if(segment == 2)  //text
			{
				...
				else //ÆÕÍ¨Ö¸Áî
				{
					...
					else if (code[line].code[0] == "mfc0")	//RÀàÐÍ
					{
						if(code[line].code.size() != 3)
						{
							cout << "Around Line " << code[line].realLine << ": " << " Parameters number wrong." << endl;
					...
					else if (code[line].code[0] == "mtc0")	//RÀàÐÍ
					{
						if(code[line].code.size() != 3)
						{
							cout << "Around Line " << code[line].realLine << ": " << " Parameters number wrong." << endl;
					...
					else if (code[line].code[0] == "addi")	//iÀàÐÍ
					{
						if(code[line].code.size() != 4)
						{
							cout << "Around Line " << code[line].realLine << ": " << " Parameters number wrong." << endl;
					...
					else if (code[line].code[0] == "addiu")	//iÀàÐÍ
					{
						if (code[line].code.size() != 4)
						{
							cout << "Around Line " << code[line].realLine << ": " << " Parameters number wrong." << endl;
					...
					else if (code[line].code[0] == "j")	//jÀàÐÍ
					{
						if(code[line].code.size() != 2)
						{
							cout << "Around Line " << code[line].realLine << ": " << " Parameters number wrong." << endl;
